        ...
        Inventory base = getConfirmGUI(forPlayer, sellIt.getItem(), new GUIElementAction() {
            ...
            public void onAction(GUIElementActionEvent e) {
                ...
                if (Config.enableTransactionLog)
                    MineBayTransactionLogger.logTransaction(seller, e.getPlayer(), r, sellIt.getPrice(), sellIt.getItem());
                e.getPlayer().closeInventory();
                if (seller.isOnline()) {
            ...
            if (e.getItemClickedWith() != null && !e.getItemClickedWith().getType().equals(Material.AIR) && e.getElementClicked() == null) {
                if (r.getOccupiedSlots() < r.getSlots() || r.getSlots() == -1) {
                    if (r.isDefaultRoom() ?
                    	...
                    	if (!r.getWhiteList().isEmpty() && !r.getWhiteList().stream().map(ItemStack::getType).collect(toCollection(ArrayList::new)).contains(e.getItemClickedWith().getType())) {
                    		e.getPlayer().sendMessage("the fuck you doing you mong");
                    		e.setCancelled(true);
                    		return;
						}
